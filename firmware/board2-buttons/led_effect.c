...
void led_effect_set(uint8_t led_id, CommButtonOutputEffect eff) {
    if (led_id < LED_EFFECT_COUNT) effects[led_id] = eff;
}

CommButtonOutputEffect led_effect_get(uint8_t led_id) {
    CommButtonOutputEffect e = { 0 };
    if (led_id < LED_EFFECT_COUNT) e = effects[led_id];
    return e;
}

static uint8_t level_for_mode(uint8_t mode) {
    switch (mode) {
        case COMM_EFFECT_MODE_DISABLED:  return 0;
        case COMM_EFFECT_MODE_ENABLED:   return 0xFF;
        case COMM_EFFECT_MODE_FLASHING:  return (phase & 0x08) ? 0x00 : 0xFF;
        case COMM_EFFECT_MODE_PULSATING: {
            uint8_t p = phase & 0x3F;                /* 0..63 */
            return (p < 32) ? (uint8_t)(p << 3)
                            : (uint8_t)((63 - p) << 3);
        }
    }
    return 0;
...
static RGBLedData color_for_value(uint8_t color, uint8_t level) {
    RGBLedData d = { 0 };
    switch (color) {
        case COMM_EFFECT_COLOR_WHITE: d.red = level; d.green = level; d.blue = level; break;
        case COMM_EFFECT_COLOR_RED:   d.red   = level; break;
        case COMM_EFFECT_COLOR_GREEN: d.green = level; break;
        case COMM_EFFECT_COLOR_BLUE:  d.blue  = level; break;
    }
    return d;
}
