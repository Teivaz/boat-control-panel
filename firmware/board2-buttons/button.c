 ...
 *   mode=RELEASE → IDLE -press→ REL_WAIT -timer→ REL_READY -release→ IDLE (fire)
 *                           -release from REL_WAIT→ IDLE (no event)
 *                           (time_ms=0 on press: skip REL_WAIT, go to REL_READY)
 */
typedef enum {
    FSM_IDLE,
    FSM_HOLD_WAIT,
...
static uint8_t button_index(const ButtonState *b) { return (uint8_t)(b - buttons); }
...
static void on_input_changed(uint8_t prev, uint8_t curr) {
    ...
    for (uint8_t i = 0; i < BUTTON_COUNT && changed; i++, changed >>= 1) {
        if (changed & 1) dispatch_edge(&buttons[i], (curr >> i) & 1);
    }
}
...
static void dispatch_edge(ButtonState *b, uint8_t pressed) {
    ...
    switch (b->mode) {
        ...
        case COMM_BUTTON_MODE_HOLD:
            if (pressed) {
                if (b->fsm != FSM_IDLE) break;          /* stray edge */
                if (b->time_ms == 0) {
                    send_button_event(i);
                    b->fsm = FSM_HOLD_FIRED;
                } else {
                ...
                if (b->fsm == FSM_HOLD_WAIT) cancel_timer(b);
                b->fsm = FSM_IDLE;
            }
            break;
