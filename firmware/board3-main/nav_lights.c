...
NavResolution nav_lights_resolve(NavMode mode, uint8_t enabled_mask) {
    NavResolution r = { 0, 0 };
    if (mode == NAV_MODE_OFF) return r;

    const NavPlan p = plans[mode];
    if ((p.primary  & enabled_mask) == p.primary)  { r.lights_mask = p.primary;  return r; }
    if ((p.fallback & enabled_mask) == p.fallback) { r.lights_mask = p.fallback; return r; }
    r.error = 1;
    return r;
}
