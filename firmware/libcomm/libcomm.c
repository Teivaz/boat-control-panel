...
uint16_t comm_button_trigger_time_ms(CommTriggerConfig config) {
    return (uint16_t) config.time_mantissa *
           comm_trigger_scale[config.time_exponent];
}
...
int8_t comm_button_effect_set(CommButtonEffect *effect, uint8_t output_index,
    ...
    if (output_index & 1) {
        bytes[byte_index] =
            (bytes[byte_index] & 0x0F) | (uint8_t) (nibble << 4);
    } else {
...
int8_t comm_button_effect_get(const CommButtonEffect *effect,
                              ...
                              CommButtonOutputEffect *value) {
    if (effect == NULL || output_index > 7 || value == NULL)
        return -1;
    const uint8_t *bytes = (const uint8_t *) effect;
    uint8_t byte_index = (7 - output_index) / 2;
    value->raw = (output_index & 1) ? (bytes[byte_index] >> 4)
