 ...
 * the hardware is designed around rather than a compromise.
 *
...
/* Configure the peripheral.  Leaves it off the bus -- call i2c_start() once
 * interrupts are enabled, or an address matched while the ISRs are masked
 * will stretch SCL indefinitely and wedge the bus for every device on it. */
void i2c_start(void);
